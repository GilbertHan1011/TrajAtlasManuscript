...
library(Seurat)
library(tidyverse)
source("../function/anno_utils.R")
full_seurat$Age <- as.character(full_seurat$Age)
full_seurat$Age[full_seurat$Age.In.Detail.%in%c("E9", "E10", "E10.5", "E11.5", "E12", "E12.5", "E13.5")] <- "Organogenesis stage"

full_seurat$Age[full_seurat$Age.In.Detail.%in%c("E14.5", "E15", "E15.5", "E16", "E16.5", "E17", "E17.5", "E18", "E18.5")] <- "Fetal stage"
...
comparisons_all_update$specificity = ((comparisons_all_update$pct.1 + specificity_base) / (comparisons_all_update$pct.2 + specificity_base)) * comparisons_all_update$avg_log2FC
comparisons_siblings_update$specificity = ((comparisons_siblings_update$pct.1 + specificity_base) / (comparisons_siblings_update$pct.2 + specificity_base)) * comparisons_siblings_update$avg_log2FC

write.csv(comparisons_all_update,"result/4.26_annotation//comparisons_all_update.csv")
write.csv(comparisons_all_update,"result/4.26_annotation/comparisons_sibling_update.csv")


#== label--------------------------------
full_seurat$C2 <- labelmat_updated_new_labels[,1]
full_seurat$C7 <- labelmat_updated_new_labels[,2]
full_seurat$C19 <- labelmat_updated_new_labels[,3]
full_seurat$C36 <- labelmat_updated_new_labels[,4]
full_seurat$C49 <- labelmat_updated_new_labels[,5]
full_seurat$C90 <- labelmat_updated_new_labels[,6]
full_seurat$C137 <- labelmat_updated_new_labels[,7]
DimPlot(full_seurat,group.by = "C2",label = T)
DimPlot(full_seurat,group.by = "C7",label = T)
DimPlot(full_seurat,group.by = "C36",label = T)
p2=DimPlot(full_seurat,group.by = "C21",label = T)
p1=DimPlot(full_seurat,group.by = "C41",label = T)
p1+p2
DimPlot(full_seurat)

train_level <- c("mix_level_1","mix_level_2","merge_id_level3","merge_id_level4","merge_id_level5")
debug(multilevel_predict)
prediction <- multilevel_predict(full_seurat,"X_scANVI",level = train_level,pred_level = "C2",subsample_size = 200)
C2_pred <- ident_predict(prediction,full_seurat$C2)
C7_pred <- ident_predict(prediction,full_seurat$C7)
C19_pred <- ident_predict(prediction_large,full_seurat$C19)

C36_pred <- ident_predict(prediction,full_seurat$C36)
C49_pred <- ident_predict(prediction_large,full_seurat$C49,threshold = 0.4)

C90_pred <- ident_predict(prediction_large,full_seurat$C90,threshold = 0.4)
C137_pred <- ident_predict(prediction_large,full_seurat$C137,threshold = 0.4)

write.csv(C2_pred,"result/4.26_annotation/4.30_integration_C2_prediction.csv")
write.csv(C7_pred,"result/4.26_annotation/4.30_integration_C7_prediction.csv")
write.csv(C19_pred,"result/4.26_annotation/4.30_integration_C19_prediction.csv")
write.csv(C36_pred,"result/4.26_annotation/4.30_integration_C36_prediction.csv")
write.csv(C49_pred,"result/4.26_annotation/4.30_integration_C49_prediction.csv")
write.csv(C90_pred,"result/4.26_annotation/4.30_integration_C90_prediction.csv")
write.csv(C137_pred,"result/4.26_annotation/4.30_integration_C137_prediction.csv")
