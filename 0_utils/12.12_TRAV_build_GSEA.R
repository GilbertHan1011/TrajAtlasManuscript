...
# dat_dir <- "~/data2/GenomicSuperSignatureLibrary/refinebioRseq"
dat_dir <- "processed_data/12.12_TRAV/geneset/"
#RAVmodel <- readRDS(file.path(dat_dir, "RAVmodel_536/refinebioRseq_RAVmodel.rds"))
...
# MSigDB C2
term2gene <- clusterProfiler::read.gmt("processed_data/12.12_TRAV/geneset/m2.all.v2023.2.Mm.symbols.gmt")
colnames(term2gene) <- c("gs_name", "entrez_gene")
...
#==C5---------------------
term2gene <- clusterProfiler::read.gmt("processed_data/12.12_TRAV/geneset/m5.all.v2023.2.Mm.symbols.gmt")
colnames(term2gene) <- c("gs_name", "entrez_gene")

for (i in seq_len(ncol(RAVmodel))) {
  fname <- paste0("gsea_", i, ".rds")
  fpath <- file.path(dat_dir, "gsea_nmf_3000", fname)
